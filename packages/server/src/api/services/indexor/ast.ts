...
export function getCodeInfoFromFile(file: string, originalCode: string, componentDefinitions: Record<string, HarmonyComponent>, elementInstances: HarmonyComponentWithNode[], importDeclarations: Record<string, {name: string, path: string}>): boolean {
    ...
    const getHashFromLocation = (location: ComponentLocation, codeSnippet: string): string => {
        ...
        return hashComponentId([{file: _file, startColumn, startLine, endColumn, endLine}]);
    }
    ...
    function createJSXElementDefinition(node: t.JSXElement, parentElement: HarmonyComponent | undefined, containingComponent: HarmonyComponent, path: string, snippet: string): HarmonyComponentWithNode | undefined {
                ...
                JSXElement: {
                    enter(jsPath) {
                        ...
                        function connectChildToParent(child: HarmonyComponentWithNode, parent: HarmonyComponentWithNode): HarmonyComponentWithNode {
                            ...
                            process.env.NODE_ENV === 'development' && console.log(`Connecting ${child.name} to ${parent.name} ${recurseConnectLog(parent)}`)
                        ...
                        function getComponentsBindingId(element: HarmonyComponent): string | undefined {
                            ...
                            const getId = (node: t.Node): string | undefined => {
                                ...
                                return _location ? getHashFromLocation(_location, originalCode) : undefined;
                            }
                                ...
                                if (t.isFunctionDeclaration(binding.path.node) || t.isArrowFunctionExpression(binding.path.node)) {
                                    ...
                                    binding.path.traverse({
                                        FunctionDeclaration(_path) {
                                            ...
                                            id = _location ? getHashFromLocation(_location, originalCode) : undefined;
                                        },
                            ...
                            if (!id) {
                                const definition = componentDefinitions[element.name] as HarmonyComponent | undefined
                                id = definition?.id;
                            }
                        ...
                        if (jsxElementDefinition) {
                            const createPropertyAttribute = (node: t.Node, type: AttributeType, name: string | undefined, propertyName: string | undefined, locationType: string): Attribute => {
                                if (!node.start || !node.end) throw new Error(`Invalid start and end for node ${String(node)}`);
                                const _location: ComponentLocation = {
                                    file,
                                    start: node.start,
                                    end: node.end
                                }
                                return createAttribute(type, 'property', name, propertyName, locationType, _location);
                            }
                            ...
                            const createObjectPropertiesAttribute = (properties: t.Node[]): Attribute[] => {
                                const attributes: Attribute[] = [];
                                for (const property of properties) {
                                    if (t.isObjectProperty(property) && t.isIdentifier(property.key) && t.isExpression(property.value)) {
                                        ...
                                        attributes.push(...attrs.map(attr => {
                                            ...
                                            return createAttribute(attr.type as AttributeType, attr.name, _name, value, attr.locationType, attr.location)
                                        }))
                                    }
                                }
                            ...
                            const createObjectExpressionAttribute = (node: t.ObjectExpression): Attribute[] => {
                            ...
                            const createStringAttribute = (node: t.StringLiteral | t.TemplateElement | t.JSXText, type: AttributeType, propertyName: string | undefined, value: string): Attribute => {
                                ...
                                return createAttribute(type, 'string', propertyName, value, 'component', _location);
                            }

                            const createAttribute = (type: AttributeType, name: string, propertyName: string | undefined, value: string | undefined, locationType: string, _location: ComponentLocation): Attribute => {
                                ...
                                if (type === 'className' && locationType === 'props' && !propertyName?.toLowerCase().includes('class')) {
                                    return createAttribute('property', name, propertyName, value, locationType, _location);
                                }
                                
                                if (name === 'string') {
                                    return {id: '', type, name: 'string', value: type === 'className' || !propertyName ? value || '' : `${propertyName}:${value}`, reference: jsxElementDefinition, index: -1, location: _location, locationType}
                                }

                                return {id: '', type, name: 'property', value: propertyName ? `${propertyName}:${value}` : value || 'undefined', reference: jsxElementDefinition, index: -1, location: _location, locationType};
                            }
                            ...
                            for (const attr of node.openingElement.attributes) {
                                if (t.isJSXAttribute(attr)) {
                                    ...
                                    const prop = createIdentifierAttribute(attr.argument, 'property', 'harmony-spread')[0] as Attribute | undefined;
