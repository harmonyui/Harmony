...
export type HarmonyComponentWithNode = HarmonyComponent & {
  node: t.JSXElement;
};
...
export function getCodeInfoFromFile(
  ...
  const getHashFromLocation = (
    ...
    return hashComponentId([
      { file, startColumn, startLine, endColumn, endLine },
    ]);
  };
          ...
          enter(jsPath) {
            ...
            function getComponentsBindingId(
              ...
              const getId = (node: t.Node): string | undefined => {
                const location = getLocation(node, file);
                return location
                  ? getHashFromLocation(location, originalCode)
                  : undefined;
              };
            ...
            if (jsxElementDefinition) {
              const createPropertyAttribute = (
                ...
                if (!node.start || !node.end)
                  throw new Error(`Invalid start and end for node ${node}`);
              ...
              const createObjectExpressionAttribute = (
                ...
                return createObjectPropertiesAttribute(
                  node.properties,
                  type,
                  name,
                );
