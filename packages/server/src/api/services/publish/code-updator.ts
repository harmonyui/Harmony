...
export class CodeUpdator {
    ...
    public async updateFiles(updates: ComponentUpdate[]): Promise<FileUpdateInfo> {
        const elementInstances = await indexForComponents(updates.map(update => update.componentId), this.gitRepository);
    ...
    private async getChangeAndLocation(update: UpdateInfo, branchName: string): Promise<CodeUpdateInfo[]> {
        ...
        const addNewClassOrComment = async ({ location, code, newClass, oldClass, commentValue, attribute }: AddClassName): Promise<CodeUpdateInfo> => {
    ...
    private transformIntoFileUpdates(codeUpdates: CodeUpdateInfo[]): FileUpdateInfo {
        ...
        for (const update of codeUpdates) {
            ...
            const ends = codeUpdates.filter(f => f.dbLocation.end >= update.location.end);
            ends.forEach(end => {
