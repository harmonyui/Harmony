...
describe('update-cache', () => {
    const setup = (changes: {path: string, type: 'add' | 'remove' | 'change', content: string}[]) => {
        ...
        const fileCache: Record<`${string}:indexing`, string[]> & Record<`${string}:${Files}`, string>  = {
            ...
            createGitRepository() {
                return {
                    ...
                    async getContent(file, ref) {
                        ...
                        return await githubCache.getFileContents({path: file, ref: ref || 'newRef', repo: ''}) || '';
                    },
    ...
    it('should update cache when file is changed', async () => {
        ...
        const {factory, repository} = setup([{path: 'file1.tsx', type: 'change', content: changedText}]);
        ...
        expect(await gitCache.getFileContents({path: 'file1.tsx', ref: 'newRef', repo: ''})).toBe(changedText);
        expect(await gitCache.getFileContents({path: 'file2.tsx', ref: 'newRef', repo: ''})).toBe('Here is file 2');
    ...
    it('should update cache when file is added', async () => {
        ...
        const {factory, repository} = setup([{path: 'file4.tsx', type: 'add', content: addedText}]);
        await updateFileCache(factory, repository, 'oldRef', 'newRef');
        const gitCache = factory.createGithubCache();
        ...
        expect(await gitCache.getFileContents({path: 'file1.tsx', ref: 'newRef', repo: ''})).toBe('Here is file 1');
        expect(await gitCache.getFileContents({path: 'file2.tsx', ref: 'newRef', repo: ''})).toBe('Here is file 2');
        expect(await gitCache.getFileContents({path: 'file3.tsx', ref: 'newRef', repo: ''})).toBe('Here is file 3');
        expect(await gitCache.getFileContents({path: 'file4.tsx', ref: 'newRef', repo: ''})).toBe(addedText);
     });

     it('should update cache when file is added', async () => {
        const {factory, repository} = setup([{path: 'file1.tsx', type: 'remove', content: ''}]);
        await updateFileCache(factory, repository, 'oldRef', 'newRef');
        const gitCache = factory.createGithubCache();
