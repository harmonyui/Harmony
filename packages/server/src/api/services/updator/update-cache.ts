...
export async function updateFileCache(gitFactory: GitRepositoryFactory, repository: Repository, oldRef: string, newRef: string) {
    ...
    if (indexedFiles) {
        ...
        //Update contents
        await Promise.all(indexedFiles.map(async (indexedFile) => {
            const ref = updatedFiles.find(updated => updated.path === indexedFile) ? newRef : oldRef;
            const content = await gitRepository.getContent(indexedFile, ref.startsWith('http') ? undefined : ref, true);
            await gitCache.setFileContents({path: indexedFile, ref: newRef, repo: repository.name}, content);
        }));

        await gitCache.setIndexingFiles({ref: newRef, repo: repository.name}, indexedFiles);
    }
}