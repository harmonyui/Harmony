...
import {
  ...
  testCases,
} from "@harmony/util/src/utils/component.spec";
import { describe, it, expect } from "vitest";
import {
  getLocationsFromComponentId,
  hashComponentId,
} from "@harmony/util/src/utils/component";
import type { FileContentRetriever } from "./local";
import { ComponentIdUpdator } from "./local";
...
describe("Component Id Updator", () => {
  const setup = (name: keyof typeof testCases, targets: string[]) => {
    ...
    for (const target of targets) {
      const p = getLocationFromContent(
        testCases[name].oldContent,
        target.replaceAll("    ", "\t"),
      );
  ...
  it("Should successfully update component ids", async () => {
    ...
    const { componentIdUpdator, componentIds } = setup(
      "add-to-top-file",
      targets,
    );

    const mappings = await componentIdUpdator.getNewIdsForComponentsFromFile(
      "",
      componentIds,
    );
