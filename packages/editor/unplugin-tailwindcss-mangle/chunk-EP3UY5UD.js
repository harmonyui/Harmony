import { getGroupedEntries, path, pluginName } from './chunk-4S5PKHEQ.js'
import { __dirname } from './chunk-3LGYGEJT.js'
...
var WEBPACK_LOADER = path.resolve(
  __dirname,
  false ? '../../dist/core/loader.cjs' : 'core/loader.cjs',
)
var factory = (options) => {
        ...
        compiler.hooks.compilation.tap(pluginName, (compilation) => {
          NormalModule.getCompilationHooks(compilation).loader.tap(
            ...
            (_loaderContext, module) => {
              if (isExisted) {
                const idx = module.loaders.findIndex((x) =>
                  x.loader.includes('postcss-loader'),
                )
                if (idx > -1) {
            ...
            async (assets) => {
              ...
              if (groupedEntries.css.length > 0) {
                for (let i = 0; i < groupedEntries.css.length; i++) {
                  ...
                  const { code } = await cssHandler(
                    cssSource.source().toString(),
                    {
                      id,
                      ctx,
                    },
