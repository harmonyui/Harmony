    ...
    return (<>
        <InfoBox type='info'>
            <div className="hw-grid hw-grid-cols-6 hw-gap-x-6 hw-text-left hw-items-center">
                <ControlGridRow title="Parents and Children" description="Each site is organized into “parents” and “children”, much like how cities belong to a county, counties in a state, etc. Whenever you select an element, Harmony will highlight its parent. You cannot drag a child outside its parent. " control={<ParentChildSvg/>}/>
            </div>
        </InfoBox>
        <div className="hw-grid hw-grid-cols-6 hw-gap-y-4 hw-text-left">
            ...
            <ControlGridRow title="Toggle component flex" description="Harmony’s flex tool gives you the choice between more responsive code and more flexible controls. Press F to toggle the parent of the selected component between Flex and non-Flex properties. " control="[F]"/>
        </div>
    </>)
}

const ControlGridRow: React.FunctionComponent<{title: string, description: string, control: React.ReactNode}> = ({title, description, control}) => {
    return (<>
