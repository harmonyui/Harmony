...
export const HelpGuide: React.FunctionComponent<{ className?: string }> = ({
  className,
}) => {
  const { setShowGiveFeedback } = useHarmonyContext()
  const [otherPopup, setOtherPopup] = useState<
    'condensed' | 'talk' | undefined
  >()
  ...
  const popup = otherPopup ? (
    ...
    <div className='hw-flex hw-flex-col hw-gap-2 hw-mr-4'>
      <div
        className='hover:hw-cursor-pointer hover:hw-text-[#88939D] hw-text-[#11283B]'
        onClick={() => {
          setOtherPopup('condensed')
        }}
      >
        View hotkey guide
      </div>
...
const HelpGuideCondensed: React.FunctionComponent<{
  onGuideClick: () => void
}> = ({ onGuideClick }) => {
  return (
...
const TalkWithUs: React.FunctionComponent<{ onAfterSend: () => void }> = ({
  onAfterSend,
}) => {
  ...
  return (
    <div className='hw-flex hw-flex-col hw-min-w-[300px]'>
      <div className='hw-border-b'>
        <Header level={3}>Contact Us:</Header>
      </div>
      <a className='hw-text-blue-500' href='mailto:jacob@harmonyui.app'>
        jacob@harmonyui.app
      </a>
      ...
      <Button className='hw-ml-auto' onClick={onSend} loading={loading}>
        Request <SendIcon className='hw-h-5 hw-w-5' />
      </Button>
    </div>
  )
}

export const GiveFeedbackModal: React.FunctionComponent<{
  show: boolean
  onClose: () => void
}> = ({ show, onClose }) => {
  const { environment } = useHarmonyContext()
  ...
  return (
    <HarmonyModal show={show} onClose={onClose} editor>
      <Header level={3}>
        We want to hear from you! How could Harmony help you and your team? Any
        feature requests?
      </Header>
      <div className='hw-flex hw-flex-col hw-gap-2'>
        ...
        {error ? <p className='hw-text-sm hw-text-red-400'>{error}</p> : null}
        <Button className='hw-ml-auto' onClick={onSend} loading={loading}>
          Send Feedback <SendIcon className='hw-h-5 hw-w-5' />
        </Button>
      </div>
    </HarmonyModal>
  )
...
const ButtonIcon: React.FunctionComponent<ButtonIconProps> = ({
  ...
  return (
    <Icon
      className={getClass(
        'hw-h-8 hw-w-8 hw-fill-white hw-stroke-none',
        className,
      )}
      onClick={onClick}
    />
