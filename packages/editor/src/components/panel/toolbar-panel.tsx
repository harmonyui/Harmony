...
import {
  InputBlur,
  NumberStepperInput,
} from '@harmony/ui/src/components/core/input'
...
export const ToolbarPanel: React.FunctionComponent<ToolbarPanelProps> = ({
  toggle,
  onToggleChange,
}) => {
  ...
  return (
    <div className='hw-inline-flex hw-gap-2 hw-items-center hw-h-full hw-w-full hw-bg-white hw-pointer-events-auto hw-divide-x'>
      <div className='hw-flex hw-items-center hw-text-nowrap hw-gap-2 hw-mr-4'>
        <Header className='hw-font-normal' level={3}>
      ...
      {data ? (
        <>
          ...
          selectDesignerElement(selectedComponent.element).parentElement
            ?.dataset.harmonyFlex ? (
            <div className='hw-px-4'>
              <HotkeyLabel
                label='Flex'
                hotkey='F'
                value={
                  selectDesignerElement(selectedComponent.element)
                    .parentElement!.dataset.harmonyFlex === 'true'
                }
                onToggle={onFlexToggle}
              />
            </div>
          ) : null}
        </>
      ...
      <div className='hw-ml-auto' style={{ borderLeft: '0px' }}>
        ...
        <HotkeyLabel
          label={toggle ? 'Designer Mode' : 'Navigator Mode'}
          hotkey='T'
...
type ComponentTool = React.FunctionComponent<{
  data: ComponentToolData[]
  onChange: (props: ComponentToolData) => void
...
const ComponentToolComponent: React.FunctionComponent<
  ComponentToolComponentProps
> = ({ ToolComponent: Component, data, onChange, getAttribute }) => {
  if (!Component) return undefined

  return (
    <Component data={data} onChange={onChange} getAttribute={getAttribute} />
  )
}

const HotkeyLabel: React.FunctionComponent<{
  label: string
  hotkey: string
  onToggle: (value: boolean) => void
  value: boolean
}> = ({ label, hotkey, onToggle, value }) => {
  return (
    <button
      onClick={() => {
        onToggle(!value)
      ...
      className='hw-text-base hw-font-light'
    >
      {label} <span className='hw-text-gray-400'>[{hotkey}]</span>
    </button>
  )
}
...
const useToolbarTools = ({ element, fonts }: ToolbarToolsProps) => {
    ...
    () => ({
      font: fonts
        ? ({ onChange, getAttribute }) => {
            ...
            const items: DropdownItem<string>[] = fonts.map((font) => ({
              id: font.id,
              name: font.name,
              className: font.id,
            }))
            return (
              <Dropdown
                className='hw-w-[170px] hw-h-7 !hw-rounded-[3px]'
                items={items}
                initialValue={value}
                onChange={(item) => {
                  onChange({ name: 'font', value: item.id })
                }}
              />
            )
          }
      ...
      fontSize: ({ onChange, getAttribute }) => {
        ...
        return (
          <NumberStepperInput
            ...
            onChange={(_value) => {
              onChange({ name: 'fontSize', value: `${_value}px` })
            }}
          />
        )
      },
      color: ({ onChange, getAttribute }) => {
        ...
        return (
          <ColorPicker<`#${string}`>
            className='hw-h-7'
            value={HexColorSchema.parse(_data)}
            onChange={(_value) => {
              onChange({ value: _value, name: 'color' })
            }}
            container={
      ...
      backgroundColor: ({ onChange, getAttribute }) => {
        ...
        return (
          <ColorPicker<`#${string}`>
            ...
            value={HexColorSchema.parse(_data)}
            onChange={(_value) => {
              onChange({ value: _value, name: 'backgroundColor' })
            }}
            container={
              document.getElementById('harmony-container') || undefined
            }
      ...
      textAlign: ({ onChange, getAttribute }) => {
        ...
        return (
          <Button className='hw-h-7' mode='none' onClick={onClick}>
            {icon}
          </Button>
        )
      },
      spacing: ({ onChange, getAttribute }) => {
        ...
        return (
          <Popover
            buttonClass='hw-h-7'
            button={<BarsArrowDownIcon className='hw-h-7 hw-w-7' />}
          >
            <div className='hw-grid hw-grid-cols-6 hw-gap-2 hw-text-sm hw-items-center hw-font-normal'>
              ...
              <Slider
                className='hw-col-span-3'
                value={line}
                max={50}
                onChange={(value) => {
                  onChange({ name: 'lineHeight', value: `${value}px` })
                }}
              />
              ...
              <Slider
                ...
                max={50}
                onChange={(value) => {
                  onChange({ name: 'letterSpacing', value: `${value / 5}px` })
                }}
              />
      ...
      textAttr: ({ data, onChange, getAttribute }) => {
        return (
          <Popover
            button={
              <button className='hw-text-base hw-font-light'>Text</button>
            }
          >
            <div className='hw-flex hw-flex-col hw-gap-2'>
              <div className='hw-flex hw-justify-around'>
                <ComponentToolComponent
                  ToolComponent={commonTools.color}
                  data={data}
                  onChange={onChange}
                  getAttribute={getAttribute}
                />
                <ComponentToolComponent
                  ToolComponent={commonTools.fontSize}
                  data={data}
                  onChange={onChange}
                  getAttribute={getAttribute}
                />
              ...
              <ComponentToolComponent
                ToolComponent={commonTools.font}
                data={data}
                onChange={onChange}
                getAttribute={getAttribute}
              />
      ...
      borderAttrs: ({ onChange, getAttribute }) => {
        ...
        if (borderWidth.length === 1)
          borderWidth.push(borderWidth[0], borderWidth[0], borderWidth[0])
        ...
        return (
          <Popover
            buttonClass='hw-h-[25.76px]'
            button={<BorderIcon className='hw-h-[25.76px] hw-w-[25.76px]' />}
          >
            <div className='hw-flex hw-flex-col hw-gap-3 hw-w-[350px] hw-p-2'>
              ...
              <div className='hw-grid hw-grid-cols-3 hw-items-center hw-justify-center'>
                ...
                <div className='hw-col-span-2 hw-flex hw-flex-row hw-p-2 hw-items-center hw-w-full hw-space-x-3 hw-border hw-border-gray-200 hw-rounded-md hw-h-12'>
                  <ColorPicker<`#${string}`>
                    className='hw-h-7 hw-z-50'
                    value={HexColorSchema.parse(_data)}
                    onChange={(_value) => {
                      onChange({ value: _value, name: 'borderColor' })
                    }}
                    container={
                      document.getElementById('harmony-container') || undefined
                    }
              ...
              <div className='hw-grid hw-grid-cols-3 hw-items-center hw-justify-center'>
                ...
                <div className='hw-col-span-2'>
                  <Dropdown
                    className='hw-w-full hw-h-12 !hw-rounded-md hw-p-2'
                    items={borderStyles}
                    initialValue={getAttribute('borderStyle')}
                    onChange={(item) => {
                      onChange({ name: 'borderStyle', value: item.id })
                    }}
                  />
                </div>
              </div>
  ...
  const toolComponents = useMemo(
    () =>
      toolNames.reduce<Record<string, ComponentTool | undefined>>(
        (prev, curr) => {
          prev[curr] = commonTools[curr]
          return prev
        },
        {},
      ),
    [toolNames, commonTools],
  )
