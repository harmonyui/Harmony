...
const EditUnit: React.FunctionComponent<EditUnitProps> = ({ units }) => {
  ...
  return (
    <div className='hw-flex hw-gap-1 hw-text-xs'>
      {units.map((unit) => (
        <button key={unit} className='hover:hw-text-gray-900 hw-text-gray-500'>
          {unitLabels[unit]}
        </button>
      ))}
    </div>
...
const EditSpacing: React.FunctionComponent<EditSpacingProps> = ({
  ...
  return (
    <EditAttribute
      ...
      sameLine={<EditUnit units={['px', 'percent']} />}
      color={color}
    >
...
const EditDisplay: React.FunctionComponent = () => {
  ...
  const getComponent = (_display: Displays): React.ReactNode => {
    ...
    if (
      selectedComponent?.parentElement &&
      getComputedStyle(selectedComponent.parentElement).display === 'grid'
    ) {
      return (
        <>
  ...
  const initialValue: Displays = items.find((item) => item.id === display)
    ? (display as Displays)
    : 'block'
...
const EditFlexAttributes: React.FunctionComponent = () => {
  ...
  const getMode = (name: CommonTools) => (value: string) => {
    ...
    return attr === value || (attr === 'normal' && value === 'flex-start')
      ? 'primary'
      : 'secondary'
  }

  const onClick = (name: CommonTools, value: string) => () =>
  ...
  const direction = getAttribute(
    'flexDirection',
  ) as (typeof directionValues)[number]
  ...
  return (
    <>
      <EditAttribute label='Flex' sameLine={undefined}>
        <div className='hw-flex hw-gap-1 hw-items-center hw-flex-wrap'>
          {justifyValues.map((value) => (
            <Button
              key={value}
              className='hw-text-xs'
              mode={getMode('justifyContent')(value)}
              onClick={onClick('justifyContent', value)}
            >
              {getLabel(value)}
            </Button>
          ))}
        ...
        <div className='hw-flex hw-gap-1 hw-items-center'>
          {alignValues.map((value) => (
            <Button
              key={value}
              className='hw-text-xs'
              mode={getMode('alignItems')(value)}
              onClick={onClick('alignItems', value)}
            >
              {getLabel(value)}
            </Button>
          ))}
        ...
        <div className='hw-flex hw-gap-1 hw-items-center'>
          {directionValues.map((value) => (
            <Button
              key={value}
              className='hw-text-xs'
              mode={getMode('flexDirection')(value)}
              onClick={onClick('flexDirection', value)}
            >
              {getLabel(value)}
            </Button>
          ))}
      ...
      <EditAttribute label='Gap' sameLine={<EditUnit units={['px']} />}>
        <InputBlur
          value={gap === 'normal' ? '0px' : gap}
          onChange={onChange(gapDirection)}
        />
      </EditAttribute>
      <EditAttribute
        label='Wrap'
      ...
      <EditAttribute label='Size' sameLine={undefined}>
        <div className='hw-flex hw-gap-1 hw-items-center'>
          ...
          <Button
            className='hw-text-xs'
            mode={shrink === '1' ? 'primary' : 'secondary'}
            onClick={() =>
              onAttributeChange({
                name: 'flexShrink',
                value: shrink === '1' ? '0' : '1',
...
const EditGridAttributes: React.FunctionComponent = () => {
  ...
  const onChange =
    (
      name: 'gridTemplateColumns' | 'gridTemplateRows' | 'rowGap' | 'columnGap',
    ) =>
    (value: string): void => {
      ...
      <EditAttribute
        ...
        sameLine={
          <InputBlur
            className='hw-w-1/4'
            value={cols}
            onChange={onChange('gridTemplateColumns')}
          />
        }
      ></EditAttribute>
      <EditAttribute
        ...
        sameLine={
          <InputBlur
            className='hw-w-1/4'
            value={rows}
            onChange={onChange('gridTemplateRows')}
          />
        }
      ></EditAttribute>
      <EditAttribute label='Gap' sameLine={<EditUnit units={['px']} />}>
        <div className='hw-flex hw-gap-1 hw-text-xs hw-text-gray-400'>
          <div className='hw-flex-1'>
            <InputBlur
              className='hw-w-full'
              value={gapX}
              onChange={(value) => onAttributeChange({ name: 'rowGap', value })}
            />
            <div>Row</div>
          </div>
          <div className='hw-flex-1'>
            <InputBlur
              ...
              value={gapY}
              onChange={(value) =>
                onAttributeChange({ name: 'columnGap', value })
              }
            />
            <div>Column</div>
          </div>
...
const EditPosition: React.FunctionComponent = () => {
  ...
  return (
    <EditAttribute
      label='Position'
      sameLine={
        <Dropdown initialValue={position} onChange={onChange} items={items} />
      }
    >
      {position !== 'static' ? (
        <>
          {(['top', 'bottom', 'left', 'right'] as const).map((side) => (
            <div key={side} className='hw-flex-1'>
              <InputBlur
                className='hw-w-full'
                value={getAttribute(side)}
                onChange={(value) => onAttributeChange({ name: side, value })}
              />
              <div>{side}</div>
            </div>
          ))}
