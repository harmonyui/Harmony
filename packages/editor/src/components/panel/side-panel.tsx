...
const SidePanelContext = createContext<SidePanelContextProps>({panel: {id: '', content: undefined}, setPanel: () => undefined});
...
export const SidePanelProvider: React.FunctionComponent<{children: React.ReactNode}> = ({children}) => {
    ...
    return <SidePanelContext.Provider value={{panel, setPanel}}>
		{children}
...
export const SidePanel: React.FunctionComponent = () => {
    ...
    >
        <div className="hw-h-full hw-overflow-auto">
            {panel?.content}
        </div>
    </Transition>
}