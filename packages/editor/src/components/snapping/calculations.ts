...
const getExtra = (
  element: HTMLElement,
  axis: Axis,
  type: Exclude<BoundingType, "size" | "size-full">,
) => {
  if (axis === "x") {
    switch (type) {
      case "close":
...
export const getOffsetRect = (
  ...
  return {
    ...
    right:
      element.offsetLeft + element.clientWidth + border.left + border.right,
    top: element.offsetTop,
    bottom:
      element.offsetTop + element.clientHeight + border.top + border.bottom,
...
export function getBoundingClientRectParent(
  parent: HTMLElement,
  axis: Axis,
  type: BoundingType,
  scale: number,
  ...
  if (axis === "y") {
    const top =
      rect.top / scale + parseFloat($(parent).css("borderTop") || "0");
    const bottom =
      rect.bottom / scale - parseFloat($(parent).css("borderBottom") || "0");
    const height =
      (rect.bottom - rect.top) / scale -
      (parseFloat($(parent).css("borderBottom") || "0") +
        parseFloat($(parent).css("borderTop") || "0"));
    switch (type) {
      case "close":
        return top;
      case "far":
      ...
      case "size-full":
        return (
          height +
          getExtra(parent, axis, "close") +
          getExtra(parent, axis, "far")
        );
    }
  } else {
    const left =
      rect.left / scale + parseFloat($(parent).css("borderLeft") || "0");
    const right =
      rect.right / scale - parseFloat($(parent).css("borderRight") || "0");
    const height =
      (rect.right - rect.left) / scale -
      (parseFloat($(parent).css("borderLeft") || "0") +
        parseFloat($(parent).css("borderRight") || "0"));
    switch (type) {
      case "close":
        return left;
      case "far":
...
export const setSpaceForElement = (
  element: HTMLElement,
  ...
  element.style[`${space}${capitalizeFirstLetter(side)}` as unknown as number] =
    propertyValue;
};

export const getNonWorkableGap = (gapInfo: GapInfo[]): number => {
  return gapInfo.reduce(
    (prev, curr) => (curr.type === "empty" ? prev + curr.value : prev),
    0,
  );
};
...
export const getSibling = (
  ...
  const selfLocationCloseOther = getBoundingClientRect(
    element,
    otherAxis,
    "close",
    scale,
  ...
  const selfLocationFarOther = getBoundingClientRect(
    element,
    otherAxis,
    "far",
    scale,
  ...
  const selfLocation = getBoundingClientRect(
    element,
    axis,
    type,
    scale,
  ...
  let minLocation: { value: number; element: HTMLElement } | undefined =
    undefined;
  for (const sibling of children) {
    ...
    if (
      !(
        (rect[otherSideClose] < selfLocationCloseOther &&
          rect[otherSideFar] < selfLocationCloseOther) ||
        (rect[otherSideClose] > selfLocationFarOther &&
          rect[otherSideFar] > selfLocationFarOther)
      )
    ) {
      //(rect[otherSideClose] >= selfLocationCloseOther && rect[otherSideClose] <= selfLocationFarOther || rect[otherSideFar] >= selfLocationCloseOther && rect[otherSideFar] <= selfLocationFarOther) {
      const isCorrectDirection =
        side === "left" || side === "top"
          ? rect[otherSide] <= selfLocation
          : rect[otherSide] > selfLocation;
      const isCloser =
        side === "left" || side === "top"
          ? rect[otherSide] > (minLocation?.value || -Infinity)
          : rect[otherSide] < (minLocation?.value || Infinity);
...
export function getGapTypesToParent(
  element: HTMLElement,
  parent: HTMLElement,
  axis: Axis,
  side: Side,
  ...
  const gap =
    side === "close"
      ? getBoundingClientRect(element, axis, side, scale) -
        getBoundingClientRectParent(parent, axis, side, scale)
      : getBoundingClientRectParent(parent, axis, side, scale) -
        getBoundingClientRect(element, axis, side, scale);
  return getGapTypes(
    element,
    ...
    axis,
    side,
    ["margin", "other-padding"],
    gap,
  );
}

export function getGapTypesToSibiling(
  element: HTMLElement,
  sibling: HTMLElement,
  axis: Axis,
  side: Side,
  ...
  const gap =
    side === "close"
      ? getBoundingClientRect(element, axis, side, scale) -
        getBoundingClientRect(sibling, axis, otherSide, scale)
      : getBoundingClientRect(sibling, axis, otherSide, scale) -
        getBoundingClientRect(element, axis, side, scale);
  return getGapTypes(
    element,
...
function getGapTypes(
  ...
  const secondNephi667 = (
    element: HTMLElement,
    numSoFar: number,
    style: string,
  ): { value: number; element: HTMLElement }[] => {
  ...
  for (const type of types) {
    ...
    gapTypes.push(
      ...values.map(({ value, element }) => ({ type, value, element, style })),
    );
  }
...
function isElementFluid(
  elm: HTMLElement,
  side: "width" | "height",
  useFlexForHeight = true,
) {
  const harmonyFixed =
    side === "width" ? "harmonyFixedWidth" : "harmonyFixedHeight";
  ...
  if (!value) {
    ...
    wrapper.style.display =
      side === "height" && useFlexForHeight ? "flex" : "block";
    ...
    if (clone[`offset${capitalizeFirstLetter(side)}` as "offsetWidth"] === ow) {
      ...
      if (
        clone[`offset${capitalizeFirstLetter(side)}` as "offsetWidth"] !== ow
      ) {
...
export function getFitContentSize(
  ...
  if (!keepPadding) {
    ...
    if (!keepPadding) clone.style.padding = "0";
...
export type ParentEdgeInfoWithSizing = ParentEdgeInfo<ChildEdgeInfoWithSizing> &
  SizingInfo;
...
export function calculateEdgesInfoWithSizing(
  element: HTMLElement,
  scale: number,
  scaleActual: number,
  axis: Axis,
  updates: UpdateRect[] = [],
): ChildEdgeInfoWithSizing {
  const elementReal = element;
  const edgeInfo = calculateEdgesInfo(
    element,
    scale,
    scaleActual,
    axis,
  ...
  if (sizingElement instanceof HTMLImageElement) {
    ...
    const { width: _minWidth, height: _minHeight } =
      getFitContentSize(sizingElement);
    const { width: _minWidthContent, height: _minHeightContent } =
      getFitContentSize(sizingElement, true);
    minWidth = _minWidth;
  ...
  let widthType: "content" | "expand" | "fixed" =
    isImageElement(sizingElement) || !isElementFluid(sizingElement, "width")
      ? "fixed"
      : close(widthReal, minWidthContent, 0.1)
        ? "content"
        : "expand";
  let heightType: "content" | "expand" | "fixed" =
    isImageElement(sizingElement) || !isElementFluid(sizingElement, "height")
      ? "fixed"
      : close(heightReal, minHeightContent, 0.1)
        ? "content"
        : "expand";

  if (
    isImageElement(sizingElement) ||
    isImageElement(selectDesignerElementReverse(sizingElement))
  ) {
    heightType = "fixed";
    widthType = "fixed";
  }
...
export function calculateEdgesInfo(
  element: HTMLElement,
  scale: number,
  scaleActual: number,
  axis: Axis,
  ...
  const elementReal = element;
  element =
    updates.find((update) => update.element === element)?.proxyElement ||
    element;

  const otherAxis = axis === "x" ? "y" : "x";
  const children = Array.from(parent.children).filter((child) =>
    isSelectable(child as HTMLElement, scaleActual),
  ) as HTMLElement[];
  const index = children.indexOf(elementReal);

  const left = calculateAxisEdgeInfo(
    element,
    parent,
    axis,
    "close",
  ...
  const right = calculateAxisEdgeInfo(
    element,
    parent,
    axis,
    "far",
  ...
  const top = calculateAxisEdgeInfo(
    element,
    parent,
    otherAxis,
    "close",
  ...
  const bottom = calculateAxisEdgeInfo(
    element,
    parent,
    otherAxis,
    "far",
  ...
  const rectOverride = updates.find(
    (update) => update.element === element,
  )?.rect;
  const parentOverride = updates.find(
    (update) => update.element === parent,
  )?.rect;
  const midpointX =
    getBoundingClientRect(element, axis, "close", scale, rectOverride) +
    getBoundingClientRect(element, axis, "size", scale, rectOverride) / 2 -
    getBoundingClientRectParent(parent, axis, "close", scale, parentOverride);
  const midpointY =
    getBoundingClientRect(element, otherAxis, "close", scale, rectOverride) +
    getBoundingClientRect(element, otherAxis, "size", scale, rectOverride) / 2 -
    getBoundingClientRectParent(
      parent,
      otherAxis,
      "close",
      scale,
...
export function calculateAxisEdgeInfo(
  element: HTMLElement,
  parent: HTMLElement,
  axis: Axis,
  side: Side,
  ...
  const getRectOverride = (element: HTMLElement): Rect | undefined => {
    const updateRect = updates.find(
      (update) => update.element === element || update.proxyElement === element,
    );
    return updateRect?.rect;
  };

  const myStart = getBoundingClientRect(
    element,
    otherAxis,
    side,
    scale,
    getRectOverride(element),
  );
  const myEnd = getBoundingClientRect(
    element,
    otherAxis,
    otherSide,
    scale,
  ...
  const parentEdge: EdgeInfo =
    side === "close"
      ? {
          gap: parentGap,
          relation: "parent",
          edgeElement: parent,
          gapTypes: getGapTypesToParent(
            element,
            parent,
            axis,
            side,
          ...
          edgeLocation: getBoundingClientRectParent(
            parent,
            axis,
            side,
            scale,
  ...
  if (selfIndex > 0 && side === "close") {
    ...
    const newStart =
      ...
      getBoundingClientRect(
        ...
        otherSide,
        scale,
        getRectOverride(sibling),
      );
    siblingEdge = {
      gap: newStart,
      relation: "sibling",
      edgeElement: sibling,
      gapTypes: getGapTypesToSibiling(
        element,
        sibling,
        axis,
        side,
      ...
      edgeLocation: getBoundingClientRect(
        sibling,
        axis,
        otherSide,
        scale,
  ...
  const elementLocation = getBoundingClientRect(
    element,
    axis,
    side,
    scale,
  ...
  return {
    ...
    siblingEdge,
    elementLocation,
    elementLocationRelative,
    parentMidpoint,
    parentMidpointRelative,
    element,
  };
}

export function calculateParentEdgeInfoWithSizing(
  parent: HTMLElement,
  scale: number,
  scaleActual: number,
  useRectOffset: boolean,
  axis: Axis,
  updates: UpdateRect[] = [],
): ParentEdgeInfoWithSizing {
  const parentInfo = calculateParentEdgeInfo<ChildEdgeInfoWithSizing>(
    parent,
    scale,
    scaleActual,
    useRectOffset,
  ...
  const { width: minWidth, height: minHeight } = getFitContentSize(
    parent,
    true,
  );
  ...
  const widthType = close(width, minWidth, 0.1)
    ? "content"
    : isElementFluid(parent, "width")
      ? "expand"
      : "fixed";
  const heightType = close(height, minHeight, 0.1)
    ? "content"
    : isElementFluid(parent, "height")
      ? "expand"
      : "fixed";
...
export function calculateParentEdgeInfo<
  T extends ChildEdgeInfo = ChildEdgeInfo,
>(
  parent: HTMLElement,
  scale: number,
  scaleActual: number,
    ...
    for (const child of children) {
      if (!updates.find((update) => update.element === child)) {
        updates.push({
          ...
          rect: getOffsetRect(child as HTMLElement),
        });
      }
    }
  }
  ...
  const left = copy.sort(
    (a, b) => a[leftSide].parentEdge.gap - b[leftSide].parentEdge.gap,
  )[0];
  const right = copy.sort(
    (a, b) => a[rightSide].parentEdge.gap - b[rightSide].parentEdge.gap,
  )[0];
  const top = copy.sort(
    (a, b) => a[topSide].parentEdge.gap - b[topSide].parentEdge.gap,
  )[0];
  const bottom = copy.sort(
    (a, b) => a[bottomSide].parentEdge.gap - b[bottomSide].parentEdge.gap,
  )[0];
  ...
  const childrenWidth = children.reduce(
    (prev, curr) => prev + getBoundingClientRect(curr, "x", "size", scale),
    0,
  );
  const childrenHeight = children.reduce(
    (prev, curr) => prev + getBoundingClientRect(curr, "y", "size", scale),
    0,
  );
  const childrenMidpointX =
    lastChild && firstChild
      ? (getBoundingClientRect(
          ...
          getBoundingClientRect(
            ...
            "close",
            scale,
            getRectOverride(firstChild),
          )) /
        2
      : 0;
  const childrenMidpointY =
    lastChild && firstChild
      ? (getBoundingClientRect(
          lastChild,
          otherAxis,
  ...
  if (childEdgeInfo.length > 1) {
    minGapBetweenX =
      childEdgeInfo[1]![leftSide].elementLocationRelative -
      childEdgeInfo[0]![rightSide].elementLocationRelative;
    minGapBetweenY =
      childEdgeInfo[1]![topSide].elementLocationRelative -
      childEdgeInfo[0]![bottomSide].elementLocationRelative;
    gapBetweenX = minGapBetweenX;
    gapBetweenY = minGapBetweenY;
    for (let i = 2; i < childEdgeInfo.length; i++) {
      const currBetweenX =
        childEdgeInfo[i]![leftSide].elementLocationRelative -
        childEdgeInfo[i - 1]![rightSide].elementLocationRelative;
      const currBetweenY =
        childEdgeInfo[i]![topSide].elementLocationRelative -
        childEdgeInfo[i - 1]![bottomSide].elementLocationRelative;
  ...
  const proxy =
    updates.find((update) => update.element === parent)?.proxyElement || parent;
...
export function calculateFlexParentEdgeInfo<
  T extends ParentEdgeInfo = ParentEdgeInfo,
>(
  parent: HTMLElement,
  scale: number,
  scaleActual: number,
  ...
  updates: UpdateRect[] = [],
  withSizing = false,
): ParentFlexEdgeInfo<T> {
  const parentInfo = withSizing
  ...
  const remainingSpaceX =
    getBoundingClientRect(parent, "x", "size", scale, getRectOverride(parent)) -
    parentInfo.childrenWidth;
  ...
  const remainingSpaceY =
    getBoundingClientRect(parent, "y", "size", scale, getRectOverride(parent)) -
    parentInfo.childrenHeight;
...
export function calculateFlexParentEdgeInfoWithSizing(
  parent: HTMLElement,
  scale: number,
  scaleActual: number,
  useRectOffset: boolean,
  axis: Axis,
  updates: UpdateRect[] = [],
): ParentFlexEdgeInfo<ParentEdgeInfoWithSizing> {
  return calculateFlexParentEdgeInfo<ParentEdgeInfoWithSizing>(
    parent,
    scale,
    scaleActual,
    useRectOffset,
