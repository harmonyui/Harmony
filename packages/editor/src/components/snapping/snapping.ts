...
function Snapping({
  ...
  const addSnapToParent = ({
    ...
    if (from && from !== "left" && from !== "top") {
      point =
        parentEdgeInfo.edges[from].parentEdge.edgeLocationRelative - point;
    }
...
class FlexSnapping implements SnapBehavior {
  ...
  public onCalculateSnapping(
    ...
    const parentInfo = calculateFlexParentEdgeInfo(
      parent,
      1,
      scale,
      false,
...
const elementSnapBehavior = new ElementSnapping(
  absoluteUpdator,
  elementUpdator,
);
  ...
  const setCssCalculations = (
    ...
    const props: UpdateRectsProps[] = oldValue.reduce<UpdateRectsProps[]>(
      (prev, [element]) => {
        ...
        const currProp = prev.find(
          (p) => p.parentUpdate.element === currParent,
        );
        if (!currProp) {
          prev.push({
            parentUpdate: {
      ...
      interact(element)
        .draggable({
          ...
          cursorChecker: function (a, b, e) {
            ...
            selectDesignerElementReverse(e as HTMLElement).style.cursor =
              cursor;
