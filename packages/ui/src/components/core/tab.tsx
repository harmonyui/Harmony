...
export const NotificationDot: React.FunctionComponent<NotificationDotProps> = ({
  ...
  return (
    <span className='relative'>
      {children}
      <span
        className={`absolute right-0 top-0 block h-1.5 w-1.5 rounded-full ${dotColor} ring-2 ring-white`}
      />
    </span>
  )
}
...
export const TabButton: React.FunctionComponent<TabControlProps> = ({
  ...
  return (
    <div className={className}>
      <ul
        className={getClass(
          'inline-flex gap-2 border rounded-md p-1 bg-gray-100 items-center',
        )}
      >
        {items.map((item) => (
          <li key={item.id}>
