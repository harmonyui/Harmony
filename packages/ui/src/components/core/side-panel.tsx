      ...
      */}
      <div className={getClass("hw-h-full", cooper.className)}>
        ...
        <div className="hw-hidden lg:hw-fixed lg:hw-inset-y-0 lg:hw-z-50 lg:hw-flex lg:hw-w-72 lg:hw-flex-col">
          {/* Sidebar component, swap this element with another sidebar if you like */}
          <div className="hw-flex hw-grow hw-flex-col hw-overflow-y-auto hw-border-r hw-border-gray-200 hw-bg-white hw-px-6 hw-py-2 hw-gap-[5.33594px] hw-pl-[23px] hw-pt-[22px]">
            {title ? <div className="hw-flex hw-shrink-0 hw-justify-center hw-items-start hw-h-[63.9922px] hw-pt-[13px]">
              {/* <img
                alt="Your Company"
                className="hw-h-8 hw-w-auto"
                src="/logo.png"
            ...
            <nav className="hw-flex hw-flex-1 hw-flex-col">
              <ul className="hw-flex hw-flex-1 hw-flex-col hw-gap-y-7">
                <li className="hw-h-full">
                  <ul className="-hw-mx-2 hw-space-y-10 hw-flex hw-flex-col hw-h-[416.336px] hw-mt-0">
                    {items.map((item) => (
                      <SidePanelItem item={item} key={item.label}/>
                    ))}
                  </ul>
