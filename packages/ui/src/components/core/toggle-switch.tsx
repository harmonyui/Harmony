...
export const ToggleSwitch = ({label, value, onChange}: ToggleSwitchProps) => {
    ...
    <label className="hw-relative hw-inline-flex hw-items-center hw-cursor-pointer">
        <input type="checkbox" checked={value} onChange={(e) => onChange(e.target.checked)} className="hw-sr-only hw-peer"/>
        ...
        {label && <span className="hw-ml-3 hw-text-sm hw-font-medium hw-text-gray-900 dark:hw-text-gray-300">{label}</span>}
    </label>
    </>
}