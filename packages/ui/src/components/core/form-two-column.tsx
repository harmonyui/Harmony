...
export const FormTwoColumn: React.FunctionComponent<FormTwoColumnProps> = ({items, onSubmit, onCancel, showButtons}) => {
  ...
  return (
    <form onSubmit={onSubmitClick}>
      <div className="space-y-12">
				{items.map(item => <FormItem item={item} key={item.title}/>)}
      </div>
