...
describe("tailwind-merge", () => {
    describe("mergeClassesWithScreenSize", () => {
        ...
        it("Should merge classes with no breakpoints", () => {
            ...
            expectClassesToBeEqual(merged, 'p-4 ml-3 mr-2 my-1 bg-blue-400 hover:bg-yellow-300');
        })

        it("Should merge classes with existing sm and md breakpoints", () => {
            const oldClass = 'p-2 sm:p-3 ml-3 md:mb-4 bg-blue-400 sm:hover:bg-red-200';
            const newClass = 'p-4 mr-2 my-1 hover:bg-yellow-300';
            const screenSize = 1280;
            const merged = mergeClassesWithScreenSize(oldClass, newClass, screenSize);
            expectClassesToBeEqual(merged, 'p-2 sm:p-4 ml-3 mr-2 md:my-1 bg-blue-400 sm:hover:bg-yellow-300');
        });

        it("Should not merge classes with existing xl breakpoint", () => {
            const oldClass = 'p-2 sm:p-3 ml-3 md:mb-4 bg-blue-400 xl:hover:bg-red-200';
            const newClass = 'p-4 mr-2 my-1 hover:bg-yellow-300';
            const screenSize = 1279;
            const merged = mergeClassesWithScreenSize(oldClass, newClass, screenSize);
            expectClassesToBeEqual(merged, 'p-2 sm:p-4 ml-3 mr-2 md:my-1 bg-blue-400 xl:hover:bg-red-200 hover:bg-yellow-300');
        })
        ...
        it("Should not merge classes with existing breakpoints at mobile size", () => {
            ...
            const merged = mergeClassesWithScreenSize(oldClass, newClass, screenSize);
            expectClassesToBeEqual(merged, 'p-4 sm:p-3 ml-3 mr-2 my-1 md:mb-4 bg-blue-400 sm:hover:bg-red-200 hover:bg-yellow-300');
        })
        ...
        it("Should merge as 2xl for very big screen size", () => {
            const oldClass = 'p-2 sm:p-3 ml-3 md:mb-4 bg-blue-400 top-0 2xl:top-10 xl:hover:bg-red-200';
