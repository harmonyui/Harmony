...
export const TeamDisplay: React.FunctionComponent<TeamDisplayProps> = ({members}) => {
...
export const TeamGrid: React.FunctionComponent<TeamGridProps> = ({members}) => {
		...
		<HarmonyModal show={show} onClose={onClose}>
			...
			<div className="hw-grid hw-grid-cols-1 hw-gap-x-6 hw-gap-y-4 sm:hw-grid-cols-6 hw-my-2">
				<Label className="sm:hw-col-span-3" label="Name:">
					<Input className="hw-w-full" value={teamMember.name} onChange={changeProperty.formFunc('name', teamMember)}/>
				</Label>
				<Label className="sm:hw-col-span-3" label="Role:">
					<Input className="hw-w-full" value={teamMember.role} onChange={changeProperty.formFunc('role', teamMember)}/>
				</Label>
                <Label className="sm:hw-col-span-full" label="Email:">
					<Input className="hw-w-full" value={teamMember.contact} onChange={changeProperty.formFunc('contact', teamMember)}/>
				</Label>
			</div>
            {error ? <p className="hw-text-red-400 hw-text-sm">{error}</p> : null}
			<div className="hw-flex">
				<Button className="hw-ml-auto" onClick={onNewTeamMember} loading={loading}>Send Invite</Button>
			</div>
		</HarmonyModal>
	)
}